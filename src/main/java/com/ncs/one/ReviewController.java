...
public class ReviewController {
	...
	public ModelAndView rinsertf(ModelAndView mv, CompanyVO vo) {
		mv.addObject("cno",vo.getCno());
		mv.addObject("cname",vo.getCname());
		mv.setViewName("review/rinsertForm");
		return mv;
    } //rinsertf
	
	@RequestMapping(value = "/rinsert")
	public ModelAndView rinsert(HttpServletRequest request, ModelAndView mv, CompanyVO cvo, ReviewVO vo, RedirectAttributes rttr) throws IOException {
		...
		System.out.println("** Ajax Test vo1 => "+vo);
		...
		System.out.println("** Ajax Test vo2 => "+vo);
	...
	public ModelAndView rdetail(HttpServletRequest request, ModelAndView mv, CompanyVO cvo, 
        ...
        mv.addObject("cno",cvo.getCno());  
        mv.addObject("cname",cvo.getCname());  
        System.out.println("cname=> "+cvo);
		HttpSession session = request.getSession(false);
		if (session != null && (session.getAttribute("loginID") != null
				|| session.getAttribute("loginCno") != null )) {
			String loginID = null;
			if (session.getAttribute("loginCno") != null)
			loginID = (String)session.getAttribute("loginCno");
	...
	// ** 답글달기
		...
		public ModelAndView replyf(ModelAndView mv, ReviewVO vo, CompanyVO cvo) {
			mv.addObject("cno",cvo.getCno());  
	        mv.addObject("cname",cvo.getCname());  
		...
		public ModelAndView reply(HttpServletRequest request, ModelAndView mv, ReviewVO vo, RedirectAttributes rttr, CompanyVO cvo) {
			...
			mv.addObject("cno",cvo.getCno());  
	        mv.addObject("cname",cvo.getCname()); 
	        vo.setCno(Integer.parseInt(vo.getId()));
